...
|---------|----------------------|-----------|
| Левый home-row edge | ESC | LGUI (Win/Cmd) |
...
Идея — привести обе клавиатуры к одинаковой моторике для Vim и переключения языка:

- **Escape** — для выхода из Insert mode (не зависит от раскладки)
- **Ctrl** — удобный модификатор на home row (CapsLock позиция)
- **Переключение языка** — под большим пальцем правой руки

| Действие | Обычная клавиатура | Сплит-клавиатура |
|----------|-------------------|-----------------|
| Escape (vim) | CapsLock tap → скрипт → Esc | ESC/LGUI tap → ESC (прошивка) |
| Ctrl | CapsLock hold → скрипт → Ctrl | LCTRL на thumb (прошивка) |
...
**Модификаторы (Ctrl ↔ Cmd):** на macOS основной модификатор — Cmd (⌘ = LGUI). Рекомендуется поменять Control и Command для сплит-клавиатуры: **System Settings → Keyboard → Keyboard Shortcuts → Modifier Keys**. Это per-keyboard настройка.
