...
| Правый крайний thumb | RALT (переключение языка через скрипт) | Left Alt |
...
- **RALT на thumb (тап)** — переключение языка через скрипт (AHK/Karabiner). LALT на hold — модификатор Alt.
...
| Переключение языка | RAlt tap → скрипт → смена языка | RALT tap → скрипт → смена языка |
| Alt модификатор | RAlt hold → скрипт → LAlt | LALT hold (прошивка) |

**Почему нет конфликта:** сплит отправляет **ESC** напрямую (не CapsLock) — скрипт CapsLock его не перехватывает. Переключение языка идёт через **RALT** на обеих клавиатурах — один и тот же путь через скрипт.
...
; Правый Alt: tap → переключение языка (Win+Space), hold → Left Alt
*RAlt:: {
...
**Right Cmd → переключение языка (tap) / Cmd (hold):**
