...
| L4 (game) | Игровой слой | combo: mo1 + Enter + mo2 |

L3 реализован через **conditional layer** — ZMK автоматически активирует его, когда зажаты оба `mo(1)` и `mo(2)`.

L4 активируется **combo** из трёх thumb-клавиш: `mo(1)` + `Enter` + `mo(2)` — одновременное нажатие. Тот же combo выключает игровой слой обратно.
...
**Combo:** нажать одновременно **mo1 + Enter + mo2** (три thumb-клавиши: левый средний + правый крайний левый + правый средний). Тот же combo выключает игровой слой обратно.
