...
**Всегда читать `README.md`** перед выполнением любой задачи в этом репозитории.

**После любых изменений keymap/конфига — сразу обновлять `README.md`**, чтобы документация отражала текущее состояние: диаграммы слоёв, таблицы модификаторов, секции «Почему так», описание base/sym/nav/sys/game. README — источник истины для пользователя, не оставлять его рассинхронизированным с `config/jorne.keymap`.
